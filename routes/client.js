...
router.get('/history', async (req, res) => {
    try {
        ...
        console.error('Erro em GET /api/client/history:', e);
        if (process.env.NODE_ENV === 'production') return res.status(500).json({ error: 'Erro ao buscar histórico' });
...
router.get('/reviews', async (req, res) => {
    try {
        ...
        console.error('Erro em GET /api/client/reviews:', e);
        if (process.env.NODE_ENV === 'production') return res.status(500).json({ error: 'Erro ao buscar avaliações do cliente' });
        res.status(500).json({ error: e.message, stack: e.stack });
    }
});
...
router.post('/reviews', async (req, res) => {
    ...
    try {
        ...
        console.log(`Review criada: id=${result.insertId} provider=${provider_id} client=${req.session.user.id} rating=${rating}`);
